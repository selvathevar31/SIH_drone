...
def construct_target(df):
    ...
    # We need to find a reading exactly 6 hours ahead at the same location.
    # To do this robustly without interpolation, we use a self merge_asof.
    # First, calculate AQI for each row if not present, but drone usually gives pm25, pm10.
    # Assuming we calculate AQI from pm25 and pm10. For simplicity in pipeline, we use pm25 as a proxy for AQI
    # or assume the DB has an 'aqi' field. Let's ensure 'aqi' exists.
    if 'aqi' not in df.columns:
        df['aqi'] = df['pm25'] # Placeholder AQI calculation for pipeline structure

    # Create a target dataframe shifted back by 6 hours to join
    ...
    # We will sort both by timestamp
    df = df.sort_values('timestamp')
    df_target = df_target.sort_values('timestamp')

    # Tolerance of 30 minutes to find a matching future reading
    ...
    # Drop rows where we couldn't find a future target
...
def run_pipeline():
    ...
    # We do NOT execute training here because user instructed "Do not train".
    # In a real run, we would instantiate HistGradientBoostingRegressor and call .fit()
